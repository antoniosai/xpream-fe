...
<template>
    <div>
            ...
            <div class="option-box">
                ...
                <div class="option-item">
                        ...
                        <select v-on:change="handlerFilter()" v-model="filter.perPage" name="per-page" id="per-page">
                            ...
                            <option v-if="filter.status!='' || filter.status!='Released'" :value="paginate.total_page">All</option>
                        </select>
                </div>
                <div class="option-item" v-if="grantAccessConsolidate">
                    <button v-on:click="showModalDownloadAll()">Download All</button>
                </div>
                <!--div class="option-item" v-if="grantAccessConsolidate">
                    <button v-on:click="download('cjobcard')">Download All Jobcard</button>
                </div>
                <div class="option-item" v-if="grantAccessConsolidate">
                    <button v-on:click="download('cmdr')">Download All MDR</button>
                </div> 
                <div class="option-item" v-if="grantAccessConsolidate">
                    <button v-on:click="download('cmrm')">Download All MRM</button>
                </div-->            
            </div>
        </div>
        <div class="is-row" id="data-project">
            <div v-for="(pg, index) in data_pg" :key="index" class="is-col is-25" style="margin-bottom: 15px;">
                <CardTracking 
                    @sync="handlerSynchronize" 
                    @option-change="handlerSelectTracking" 
                    @modal-highlight="handlerModalHighlight"
        ...
        <div class="modal-box" id="modal-download-all">
            <div class="modal sm">
                <span class="close" v-on:click="hideModal('modal-download-all')"></span>
                    <div class="modal-header">
                        <h2 class="title"><span id="status-post"></span> Download All</h2>
                    </div>
                    <div class="modal-body" style="position: relative">
                        <div class="form-item">
                            <label for="dhgr">Hangar</label>
                            <select id="dhgr">
                            ...
                            <select id="dtype">
                                <option value="cjobcard">Jobcard</option>
                                <option value="cmdr">MDR</option>
                                <option value="cmrm">MRM</option>
                            </select>
                        </div>
                    </div>
                    <div class="modal-footer" style="">
                        <button class="button is-primary is-default cancel" v-on:click="hideModal('modal-download-all')">Close</button>
                        <button class="button is-success" type="submit" v-on:click="handlerDownloadAll()" id="btn-save-download-all">Download</button>
                    </div>
            </div>
        </div>
        ...
        <div class="modal-box" id="modal-project-status">
            <div class="modal sm">
                <span class="close" v-on:click="hideModal('modal-project-status')"></span>
                    <div class="modal-header">
                        <h2 class="title"><span id="status-post"></span> Project Status #{{form_status.pg_id}}</h2>
                    </div>
                    <div class="modal-body" style="position: relative">
                        <div class="form-item">
                            <label for="">Status</label>
                            <select v-model="form_status.status" name="project-status" id="project-status">
                                <option value="">Select Status</option>
                                <option v-for="(stat, index) in status" :key="index" :value="stat.ID">{{stat.NAME}}</option>
                            </select>

                        </div>
                    </div>
                    <div class="modal-footer" style="">
                        <button class="button is-primary is-default cancel" v-on:click="hideModal('modal-project-status')">Close</button>
                        <button v-if="this.grantAccessStatus" class="button is-success" v-on:click="handlerPostStatus(form_status.index)" type="submit" id="btn-save-status">Save</button>
        ...
        <div class="modal-box" id="change_date_modal">
            <div class="modal md">
                ...
                <div class="modal-body">
                    <PGDetails v-bind:pg="form_date.pg_data" />
                    <br>
                    <div class="card">
                        <div class="is-row">
                            <div class="is-col is-50">
                                <label>Start Date</label>
                                <input type="date" v-model="form_date.pg_data.start_date" disabled>
                                <hr>
                                <label>Start Date New</label>
                                <input type="date" v-model="form_date.pg_data.start_date_new">
                            </div>
                            <div class="is-col is-50">
                                <label>End Date</label>
                                <input type="date" v-model="form_date.pg_data.end_date" disabled>
                                <hr>
                                <label>End Date New</label>
                                <input type="date" v-model="form_date.pg_data.end_date_new">
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button id="btn-save-default" class="button is-primary" v-on:click="changeDateToDefault">
                        Change to Default
                    </button>
                    <button id="btn-save-date" class="button is-success"
...
export default {
    ...
    methods: {
        ...
        download: function(item, hgr){
            var link = '';
            var downloadname = '';
            if(item=='cjobcard'){
                link = '/api/production_control_hangar/consolidate_jobcard';
                downloadname = 'jobcard_tracking';
            }
            else if(item=='cmdr'){
                link = '/api/production_control_hangar/consolidate_mdr';
                downloadname = 'mdr_tracking';
            }
            else if(item=='cmrm'){
                link = '/api/production_control_hangar/consolidate_mrm';
                downloadname = 'mrm_tracking';
            }

            if(link!=''){
                this.loading.isLoading = true;
                axios.post(link, {hgr: hgr})
                    .then(res => {
                        this.handlerExport(res.data, downloadname).then(() => {
        ...
        handlerExport: function(data, name) {
            return new Promise((resolve, reject) => {
                ...
                this.hideModal('modal-download-all')
                resolve();
            })
        },
        ...
        initData: function() {
            return new Promise((resolve) => {
                this.loading.isLoading = true;
                this.filter.page = 1;
                axios.post('/api/production_control_hangar/get_list_tracking', this.filter)
                .then(res => {
        ...
        handlerSyncJMRM: function(revnr) {
            return new Promise((resolve, reject) => {
                this.syncMRMjcmdr(revnr).then((res) => {
                    if(res === 'success') {
                        this.syncMRMpdsheet(revnr).then((res) => {
                            if(res === 'success') {
                                this.syncMRMsloc(revnr).then((res) => {
                                    if(res === 'success') {
        ...
        syncMRMjcmdr: function(revnr) {
            return new Promise((resolve, reject) => {
                toastr.info('start sync material item from jobcard & MDR');
                this.loading.isLoading = true;
                axios.get('/api/production_control_hangar/sync_mrm_jcmdr/' + revnr)
                .then(res => {
                    if(res.data.success) {
        ...
        syncMRMpdsheet: function(revnr) {
            return new Promise((resolve, reject) => {
                toastr.info('start sync material item from pdsheet');
                this.loading.isLoading = true;
                axios.get('/api/production_control_hangar/sync_mrm_pdsheet/' + revnr)
                .then(res => {
                    if(res.data.success) {
        ...
        syncMRMsloc: function(revnr) {
            return new Promise((resolve, reject) => {
                toastr.info('updating material store location & workcenter');
                this.loading.isLoading = true;
                axios.get('/api/production_control_hangar/sync_mrm_sloc/' + revnr)
                .then(res => {
                    if(res.data.success) {
        ...
        showModalDownloadAll: function() {
            $("#modal-download-all").show();
        },

        handlerDownloadAll: function() {
            let dhgr = $('#dhgr').val()
            let dtype = $('#dtype').val()

            // console.log(dhgr, dtype)

            this.download(dtype, dhgr)
            ...
            this.handleButton('btn-save-highlight', 'load').then(() => {
                ...
                .then(res => {
                    if(res.data.success) {
                        ...
                        this.data_pg[this.form_highlight.index].project_highlight = this.form_highlight.highlight;
                ...
                if (value) {
                    ...
                    this.handleButton('btn-save-default', 'load').then(() => { 
                        ...
                        .then( res => {
                            ...
                            this.handleButton('btn-save-default', 'stop', 'primary');
                        })
                        .catch(() => {
                            this.handleButton('btn-save-default', 'stop', 'primary');
                        })
            ...
            .then((value) => {
                if (value) {
                    ...
                    this.handleButton('btn-save-date', 'load').then(() => { 
                        let params = {
                            pg_id: this.form_date.change_date.pg_id,
                            start_date_new: this.form_date.change_date.start_date_new,
                            end_date_new: this.form_date.change_date.end_date_new
                        }
            ...
            .then((value) => {
                if (value) {
                    ...
                    this.handleButton('btn-save-status', 'load').then(() => {
                        ...
                        axios.post('/api/production_control_hangar/update_project_status', {
                        ...
                        .then( res => {
                            if(res.data.success) {
                                toastr.success(res.data.message);
                                const idx = this.status.findIndex(elm => elm.ID === this.form_status.status);
                                this.data_pg[this.form_status.index].custom_status = this.status[idx].NAME;
                                ...
                                toastr.error(res.data.message, 'Failed', {timeOut: 15000});
                                setTimeout(()=> {
                                    this.hideModal('modal-project-status')
                                }, 500);
                                setTimeout(()=> {
