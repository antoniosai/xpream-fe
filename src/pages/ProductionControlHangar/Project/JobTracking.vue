        ...
        <div class="content-body" style="padding-top:40px;">
            <ul class="tab-menu">
                ...
                <!-- <li
                    v-if="
                        $bulkCan(
                            'pc_hangar_tracking_system_view_prm',
                            'pc_hangar_tracking_system_edit_prm',
                            'pc_hangar_tracking_system_edit',
                            'pc_hangar_tracking_system_view'
                        )
                    "
                                        ...
                                        >
                                            ...
                                            >
                                                <div style="text-align:center;">
                                                    <span 
                                                        :id="'mdr-raised-' + props.index"
                                                        statetoggle="0"
                                                        class="mdr-raised"
                                                        v-on:click="handlerToggleMDR('mdr-raised-' + props.index, 'mdr-list-' + props.index)"
                                                    >
                                                    </span>
                                                </div>
                                                <div
                                                    :id="'mdr-list-' + props.index"
                                                    class="mdr-list"
                                                    style="margin-top: 5px; display: none;"
                                                >
                                                    <div
                                                        :id="'mdr-list-card-' + props.index"
                                                        class="card matcard"
                                                    > 
                                                    </div>
                                                </div>
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                >
                                    ...
                                    <td>
                                        <button
                                            @click="dataplansliporder.MRM.splice(index2, 1)"
                                            class="button is-danger"
                                        >
                                            Delete
                                        </button>
...
const GET_MDR_FROM_JOBCARD = gql`
    query MdrFromJobcard($revnr: String!) {
        XPREAM_MDRTRACKING_VIEW(where: {REVNR: {_eq: $revnr}}) {
            ORIGINATING_ORDER
            DESCRIPTION
            AUFNR
            SYSTEM_STATUS
...
export default {
    ...
    methods: {
        getCountMdrByJobcard() {
            this.$apollo.query({
                query: GET_MDR_FROM_JOBCARD,
                variables: {
                    revnr: this.$route.params.pg_id
                }
            }).then((response) => {
               // console.log(response.data.XPREAM_MDRTRACKING_VIEW)
               this.mdrfromjc = response.data.XPREAM_MDRTRACKING_VIEW
               this.datatable.rows.forEach(this.matchmdrbyjobcard)
            })
        },

        matchmdrbyjobcard: function(item, index) {
            let elem = $("#mdr-raised-" + index)
            elem.removeAttr("class").text('')
            console.log(elem)
            let mdr = this.mdrfromjc.filter(d => d.ORIGINATING_ORDER === item.AUFNR)
            if (mdr.length > 0) {
                elem.addClass("status-handler primary").text(mdr.length);
                let stringhtml = "<ul>"
                mdr.forEach(m => {
                    stringhtml += '<li>' + m.AUFNR + ' [' + 
                    (m.TRACK_STATUS ? m.TRACK_STATUS : 'OPEN')
                    + '] - ' + m.DESCRIPTION + '</li>'
                })
                stringhtml += "</ul>"
                $("#mdr-list-card-" + index).html(stringhtml)
            }
            /*this.dfp_url.find(a =>
        ...
        handlerToggleMDR: function(btn_id, elem_id) {
            let det_content = document.getElementById(elem_id);
            ...
            try {
                ...
                if (message === "Cant create daily menu due to expired date") {
                    ...
                    if (!status) {
                        ...
                        axios
                            .post(
                                "/api/production_control_hangar/get_tjobcard",
        ...
        slip_printout: function() {
            ...
            if (this.pagebreak) {
                var style = "<style>@media print{.detectpagebreak {page-break-after:always}"
                style += "tr {page-break-inside:avoid; page-break-after:auto} td { page-break-inside:avoid; page-break-after:auto } thead { display:table-header-group } tfoot { display:table-footer-group }}"
                style += "@page { size: auto;  margin-bottom: 5mm; } body{-moz-transform: scale(0.7, 0.7);zoom: 0.7;zoom: 70%;} .materialitem table, .materialitem td, .materialitem th{ border: 1px solid black;}</style>"
            } else {
                var style = "<style>@media print{.detectpagebreak {page-break-after:auto;  page-break-inside:avoid;}"
                style += "tr {page-break-inside:avoid; page-break-after:auto} td { page-break-inside:avoid; page-break-after:auto } thead { display:table-header-group } tfoot { display:table-footer-group }}"
                style += "@page { size: auto;  margin-bottom: 5mm; } body{-moz-transform: scale(0.7, 0.7);zoom: 0.7;zoom: 70%;} .materialitem table, .materialitem td, .materialitem th{ border: 1px solid black;}</style>"
            }
        ...
        initData: function() {
            ...
            axios
                ...
                .then(res => {
                    ...
                    this.datatable.totalRows = res.data.total;
                    this.loading.isLoading = false;
                    setTimeout(() => this.datatable.rows.forEach(this.matchmdrbyjobcard), 500);
                })
                .catch(e => {
                    console.log("Error: " + e);
                    toastr.error("Undefined error");
                ...
                axios
                    ...
                    .then(res => {
                        if (res.data.length !== FieldJobcard.length) {
                            ...
                            if (this.is_cust) {
                                this.datatable.fields = res.data.filter(d => 
                                        d.label !== 'System Status' && 
                                        d.label !== 'MH' &&
                                        d.label !== 'MAT' &&
                                        d.label !== 'Pmhrs' &&
        ...
        initTrack: function() {
            axios.get("/api/management/job_mdr_track/get/hangar").then(res => {
                this.tracks = res.data.filter(d => d.TRACK_NAME !== 'NEED RO' && d.TRACK_NAME !== 'WAITING ATTACHMENT' && d.TRACK_NAME !== 'WAITING NEXT RO');
            });
        },
        ...
        handleDatatableChange: function(params) {
            ...
            return new Promise((resolve, reject) => {
                axios
                    .post("/api/production_control_hangar/get_tjobcard", params)
                    .then(res => {
                        this.datatable.rows = res.data.data;
                        this.datatable.totalRows = res.data.total;
                        this.loading.isLoading = false;
        ...
        handlerSyncData: function() {
            swal({
                ...
                if (value) {
                    ...
                    axios
                        .get(
                            "/api/production_control_hangar/sync_jobcard/" +
                ...
                if (res) {
                    ...
                    axios
                        ...
                        .then(res => {
                            ...
                            this.actypeinfo = res.data.info.ac_type;
                            this.lineinfo = res.data.info.line_id;
                            if (
                                this.lineinfo == '1' || 
                                this.lineinfo == '4' ||
                                this.lineinfo == '12' ||
                                ...
                                this.sloc = '2070'
                            } else if (this.lineinfo == '19') { this.sloc = '2080' }
                            else if (this.lineinfo == '20') { this.sloc = '2090' }
                            else if (this.lineinfo == '21') { this.sloc = '2100' }
                            else if (this.lineinfo == '22') { this.sloc = '2110' }
                            else if (this.lineinfo == '23') { this.sloc = '2120' }
                            else if (this.lineinfo == '24') { this.sloc = '2130' }
                            else if (this.lineinfo == '25') { this.sloc = '2140' }
                            else if (this.lineinfo == '26') { this.sloc = '2150' }
                              
                            this.dfpfolder = res.data.info.dfpfolder;
                            //console.log(this.dfpfolder);
                            this.handlergetdfp(this.dfpfolder);
                            this.isPKG =
                                res.data.info.ac_reg.substring(0, 4) === "PK-G"
                                    ? true
                                    : false;
