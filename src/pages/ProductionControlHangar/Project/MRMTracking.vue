<style scoped>
    ::v-deep .btn-custom-toggle {
        height: 30px;
        margin-right: -2.5px;
    }

    ::v-deep .is-secondary-toggle {
        background-color:#ededed !important ;
        color: rgb(192, 192, 192) !important;
    }

    ::v-deep .is-primary-toggle-warning {
        background-color:#e26c2b !important ;
        color: rgb(255, 255, 255) !important;
    }

    ::v-deep .is-primary-toggle-danger {
        background-color:#e22b2b !important ;
        color: rgb(255, 255, 255) !important;
    }

    ::v-deep .is-secondary-toggle:hover {
        opacity:0.5;
    }
    ::v-deep .slochandler {
        color: black;
    ...
    <div v-show="hasAccess">
        ...
        <div class="content-body" style="padding-top:40px;">
            <ul class="tab-menu">
                ...
                <li v-if="$bulkCan('pc_hangar_tracking_system_view_mrm', 'pc_hangar_tracking_system_edit_mrm', 'pc_hangar_tracking_system_edit', 'pc_hangar_tracking_system_view')" class="tab-link active" data-tab="mrm">MRM</li>
                ...
                <li v-if="$bulkCan('pc_hangar_tracking_system_view_crm', 'pc_hangar_tracking_system_edit_crm')" v-on:click="$router.push({name: 'pc_hangar.tracking.crm', params: {pg_id: $route.params.pg_id}})" class="tab-link" data-tab="crm">CRM</li>
                                    ...
                                    >
                                        ...
                                        <div slot="table-actions">
                                            <div class="toolbar toolbar-default">
                                                ...
                                                <div class="toolbar-item tooltip-x" style="margin-right: -10px;">
                                                    <button @click="handlerSyncData" class="button is-danger">
                                                        <i class='material-icons'>
                                                            sync
                                                        </i>
                                                        Sync
                                        ...
                                        <template slot="table-row" slot-scope="props">
                                            ...
                                            <span class="cprior" v-else-if="props.column.field == 'URGENCY'">
                                                ...
                                                "-->
                                            <button type="button" @click="changePriority(props.row, '')" :class="`button btn-custom-toggle ${props.row.URGENCY === '' ? 'is-secondary-toggle' : 'is-secondary-toggle'}`" :style="'padding:4px;border-radius:10px 0px 0px 10px;'">low</button>
                                            <button type="button" @click="changePriority(props.row, '0')" :class="`button btn-custom-toggle ${props.row.URGENCY === '0' ? 'is-primary-toggle-warning' : 'is-secondary-toggle'}`" :style="'padding:4px;border-radius:0px;'">med</button>
                                            <button type="button" @click="changePriority(props.row, '1')" :class="`button btn-custom-toggle ${props.row.URGENCY === '1' ? 'is-primary-toggle-danger' : 'is-secondary-toggle'}`" :style="'padding:4px;border-radius:0px 10px 10px 0px;'">high</button>
                                                    <!--input :disabled="!grant_access" type="checkbox" :class="'switchprio unset'" @change="changePriority(props.row)" v-model="props.row.URGENCY"-->
                                                <!--/span-->
                                            </span>                                            
                                            ...
                                            <span v-else-if="props.column.field == 'TOTAL_QTY_REQ'">
                                                <span :id="'qty-all-count-' + props.index"></span>
                                            </span>
                                            ...
                                            <span v-else-if="props.column.field == 'MATFULSTAT'">
                                                <div style="margin-top: 6px; text-align: center;">
                                                    ...
                                                    <span style="background-color: #2ECC71;" class="status-handler" v-else-if="props.formattedRow[props.column.field] === 'Pending / No Action'">
                                                        <span v-on:click="handlerModalTimeline(props.formattedRow['ID'])" style="cursor: pointer;">
                                                            {{props.formattedRow[props.column.field]}}
                                                        </span>
                                                    </span>
                                            ...
                                            <span v-else-if="props.column.field == 'ADDITIONAL_STATUS'">
                                                <select v-model="datatable.rows[props.index].ADDITIONAL_STATUS" :disabled="!grant_access" :id="'mrm-track-additional-status-' +props.index" 
...
const GET_MRM_SUMQTY = gql`
    query MdrFromJobcard($revnr: String!) {
        XPREAM_MRMTRACKING_VIEW(where: {REVNR: {_eq: $revnr}, VISIBILITY: {_eq: "y"}}) {
            AUFNR
            MATNR
            BDMNG
        }
...
export default {
    ...
    methods: {
        getSumQtyMRM() {
            // console.log('tes')
            this.$apollo.query({
                query: GET_MRM_SUMQTY,
                variables: {
                    revnr: this.$route.params.pg_id
                }
            }).then((response) => {
               // console.log(response.data.XPREAM_MRMTRACKING_VIEW)
               this.allitemqty = response.data.XPREAM_MRMTRACKING_VIEW
               this.datatable.rows.forEach(this.matchmatnr)
            })
        },

        matchmatnr: function(item, index) {
            $("#qty-all-count-" + index).text('x')
            
            let mat = this.allitemqty.filter(d => d.MATNR === item.MATNR)
            if (mat.length > 0) {
                // console.log(mat)
                let sum = mat.map(item => item.BDMNG).reduce((prev, next) => prev + next)
                // console.log(sum)
                $("#qty-all-count-" + index).text(sum.toFixed(2))
            }
        },
        ...
        initData:  function() {
            ...
            .then(res => {
                ...
                setTimeout(() => this.datatable.rows.forEach(this.matchmatnr), 500);
        ...
        initArangedLayout: function() {
            return new Promise((resolve) => {
                ...
                .then(res => {
                    if(res.data.length !== FieldMRM.length) {
                        this.datatable.fields = FieldMRM.map((l) => Object.assign({}, l));
                    }else {
        ...
        handleDatatableChange: function(params) {
            ...
            return new Promise((resolve, reject) => {
                ...
                axios.post('/api/production_control_hangar/get_tmrm', params)
                .then(res => {
                    this.datatable.rows = res.data.data
                    this.datatable.totalRows = res.data.total
                    this.loading.isLoading = false;
        ...
        handlerDateColumns: function(e) {
            ...
            if(el_1.length){el_1[0].type = 'date';}
            if(el_2.length){el_2[0].type = 'date';}
            if(el_3.length){el_3[0].type = 'date';}
            //if(el_4.length){el_4[0].type = 'date';}
            if(el_5.length){el_5[0].type = 'date';}
            if(el_6.length){el_6[0].type = 'date';}
            if(el_7.length){el_7[0].type = 'date';}  
        ...
        handlerExportupdate: function(data, status, name) {
            return new Promise((resolve, reject) => {
                let note = [
                    {COLUMN:"Toggle", RULE:"y or n"},
                    {COLUMN:"Part Number", RULE:"*required"},
                    {COLUMN:"Order", RULE:"*required"},
                    {COLUMN:"Mat Supply", RULE:"CSP or GMF"},
                    {COLUMN:"Additional Status", RULE:"Input ID Number from ADDITIONAL_STATUS_ID Sheet"},
                    {COLUMN:"Date of PO", RULE:"yyyy-mm-dd"},
                    {COLUMN:"ETA", RULE:"yyyy-mm-dd"},
                    {COLUMN:"Date of Provision", RULE:"yyyy-mm-dd"},
                    {COLUMN:"Date at GAH4 1000", RULE:"yyyy-mm-dd"},
                    {COLUMN:"Date PRE-LOAD", RULE:"yyyy-mm-dd"},
                    {COLUMN:"", RULE:""},
                    {COLUMN:"input value 'NULL' to clear/delete existing data", RULE:""}
                ];
        ...
        changePriority: function (list, val) {
            this.loading.isLoading = true;
            let data_update = "";
