    ...
    <div>
                     ...
                     <transition name="slide-fade">
                            <div class="is-row">
                                <div class="is-col">
                                    <div class="form-item">
                                        <label>Date :</label>
                                        <input type="date" id="dateFilter" v-model="filterBarcode.dateFilter" size="30" style="border: 1px solid rgb(234, 234, 234); background: rgb(255, 255, 255);">
                                    </div>
                                </div>
                                <div class="is-col">
                                    <div class="form-item">
                                        <label>Manhour Status :</label>
                                        <select id="statusFilter" v-model="filterBarcode.statusFilter">
                                ...
                                <div class="is-col">
                                    <div class="form-item">
                                        <label for="">&nbsp;</label>
                                        <button v-on:click="getProject()" class="button is-primary is-success"><i class="material-icons">search</i> Search</button>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="content-body">
            ...
            <div class="card">
                <div class="card-header no-border">
                    <div class="title-area">
                        <h3 class="title">Gantt Chart #{{ $route.params.pg_id }}/{{ filterBarcode.dateFilter }}/{{ filterBarcode.statusFilter }}</h3>
                    </div>
                </div>
                <div class="card-content">
                    <table class="is-bordered is-striped">
                        <thead>
                            <tr>
                                <th style="text-align: center;" rowspan="2" width="8%">Order</th>
                                <th style="text-align: center;" rowspan="2" width="8%">Description</th>
                                ...
                                <th style="text-align: center" rowspan="2" width="4%">Pmhrs</th>
                                <th rowspan="2" style="text-align: center" width="5%">Act Mhrs</th>
                        ...
                        <tbody>
                            <tr v-for="(row, index) in pg" :key="index" >
                                <td style="text-align: center;"><b>{{row.order_no}}</b></td>
                                <td>{{row.order_desc}}</td>
                                <td style="text-align: center;">{{row.order_type}}</td>
                                <td>
                                    <div style="margin-left: 6px; padding: 5px 0px 5px 0px;">
                                        <div v-if="row.skill_set !== null" v-html="handlerSkillColorResume(row.skill_set)"></div>
                                    </div>
                                </td>
                                <td style="text-align: center;">{{row.plan_mhrs}}</td>
                               ...
                               <td>
                                    <div style="padding: 5px 0px 5px 0px;">
                                        <div v-for="(power, indexactmp) in row.act_manpower" :key="indexactmp">
                                            <div style="font-size: 9pt; text-align: center;" :class="'card card-power-selected ' + handlerColorManpower(power.SKILLCATEG)">
                                                {{power.NAMA}} ({{power.SKILLCATEG}})
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td style="text-align: center;">
                                    <div style="padding: 3px 0px 3px 0px;" v-for="(power, indexactmp) in row.act_manpower" :key="indexactmp">
                                        <span style="background-color: #13CE66; font-size: 9pt; width: 72px;" v-if="power.BARCODE_STATUS === 'START'" class="label is-success">RUNNING</span>
                                        <span style="background-color: #6c757d; font-size: 9pt; width: 72px;" v-else-if="power.BARCODE_STATUS !== null && power.BARCODE_STATUS !== 'START'" class="label is-success">STOP</span>
                                    </div>
                                </td>
                                <td>
                                    <div v-html="mhrsstatus(row.act_manpower,row.plan_mhrs)" style="padding: 5px 0px 5px 0px;"></div>
                                </td>
...
export default {
    ...
    mounted() {
        ...
        this.filterBarcode.dateFilter = date;
        this.filterBarcode.statusFilter = 'All';
        this.getProject();
        // this.forceRerender();
    },
    ...
    methods: {
        ...
        getProject: function() {
            ...
            var pg_id = this.$route.params.pg_id;
            var plan_date = this.filterBarcode.dateFilter;
            var mhrs_status = this.filterBarcode.statusFilter;

            return new Promise((resolve, reject) => {
                axios.get('/api/get_gantt_daily_report_barcode_filter/' + pg_id + '/' + plan_date + '/' + mhrs_status )
                .then( res => {
                    if (res.data.success == 1) {
                        var requestStatus = res.data.request_mhrs_status;
                        var result = res.data.data;

                        if( requestStatus == 'Normal'){
                            this.pg = result.filter(function(result) {
                                return result.status_mhrs == 'Normal';
                            });
                        }else if(requestStatus == 'Abnormal'){
                            this.pg = result.filter(function(result) {
                                return result.status_mhrs == 'Abnormal';
                            });
                        }else{
                            this.pg = result;
                        }
        ...
        initGanttJob: function() {
            return new Promise((resolve, reject) => {
                axios.get('/api/booking_and_submission/get_gantt_job/' +this.pg_data.f_pg_id + '/' + this.pg.hangar.name)
                .then(res => {
        ...
        initResume: function() {
            ...
            return new Promise((resolve, reject) => {
                axios.get('/api/booking_and_submission/get_gantt_list/' + this.pg_data.f_pg_id + '/' + this.pg_data.f_date + '/' +shift + '/' + this.pg.hangar.name + '/' + this.pg_data.f_type.toUpperCase())
                .then(res => {
        ...
        initDataReport: function() {
            return new Promise((resolve, reject) => {
                axios.get(`/api/booking_and_submission/get_gantt_daily_report/${this.daily_report.f_pg_id}/${this.daily_report.f_date}/${this.daily_report.f_shift === 'shift-1' ? 1 : 2}/${this.pg.hangar.name}/${this.daily_report.f_type.toUpperCase()}`)
                .then(res => {
                    this.d_reports = res.data;
                    // this.d_reports = res.data;
                    // this.jumlah = this.d_reports.act_manpower.reduce((sum, curr) => sum + curr.ACT_MHRS, 0);
        ...
        initDataReportBarcode: function() {
            return new Promise((resolve, reject) => {
                axios.get(`/api/booking_and_submission/get_gantt_daily_report_barcode/${this.daily_report.f_pg_id}/${this.daily_report.f_date}/${this.daily_report.f_shift === 'shift-1' ? 1 : 2}/${this.pg.hangar.name}/${this.daily_report.f_type.toUpperCase()}`)
                .then(res => {
                    resolve(res.data);
                })
                .catch(e => {
        ...
        initDataReportAdditional: function() {
            return new Promise((resolve, reject) => {
                axios.get(`/api/booking_and_submission/get_gantt_daily_report_additional/${this.daily_report.f_pg_id}/${this.daily_report.f_date}/${this.daily_report.f_shift === 'shift-1' ? 1 : 2}/${this.pg.hangar.name}/${this.daily_report.f_type.toUpperCase()}`)
                .then(res => {
                    resolve(res.data);
                })
                .catch(e => {
        ...
        handlerGanttMaterial: function(rev_no, order_no, index) {
            ...
            return new Promise((resolve, reject) => {
                axios.get(`/api/booking_and_submission/get_gantt_material/${rev_no}/${order_no}/${this.pg.hangar.name}`)
                .then(res => {
        ...
        handlerModalTools: function(rev_no, order_no, index) {
            ...
            this.handlerInitDataTools(order_no).then(() => {
        ...
        updateParams(newProps) {
            this.m_tools.serverParams = Object.assign({}, this.m_tools.serverParams, newProps);
        },
        ...
        handlerSkillColorResume: function(data_skill){
            let result = "";
            var stringFirst = data_skill.charAt(0);
            if( stringFirst == ','){
                var data_skill = data_skill.slice(1);
            }

            // console.log("data skill = " + data_skill);
            // console.log("first string = " + stringFirst);
            if(data_skill !== null) {
                let fix_skill = data_skill.split(' ').join('');

                let arr_skill = fix_skill.split(",");
                console.log("array skill = " + arr_skill);
                for(let i = 0; i < arr_skill.length; i++) {
                    let temp_skill = "";
                    switch(arr_skill[i]) {
                        case "A1":
        ...
        handlerColorManpower: function(skill) {
            let result = "";
            switch(skill) {
                case "A1":
        ...
        handlerAddReport: function() {
            ...
            for(let i = 0; i < this.m_order.selected.length; i++) {
                ...
                if(temp_order.length > 0) {
                    toastr.warning(`Order ${this.m_order.selected[i].order_no} already exists`);
                }else {
        ...
        handelrFindOrder: function(query, order) {
            if(query.length >= 4) {
                ...
                this.m_order.items = []
                axios.get(`/api/booking_and_submission/get_gantt_job_by/${this.daily_report.f_pg_id}/${query}/${this.pg.hangar.name}`)
                .then(res => {
        ...
        handlerSaveDailyReport: function() {
            ...
            axios.post('/api/booking_and_submission/save_gantt_daily_report', {
        ...
        handlerPushDailyReport: function(data) {
        ...
        exportGantt: function(divName) {
            ...
            gantt.exportToPDF({
                ...
                footer:`${this.handlerFooter(
                    this.pg_position.project_owner.name !== null ? this.pg_position.project_owner.name : "&emsp;&emsp;&emsp;",
                    this.pg_position.project_sponsor.name !== null ? this.pg_position.project_sponsor.name : "&emsp;&emsp;&emsp;"
                )}`,
                locale:"en",
                skin:'terrace',
                raw:true
        ...
        handlerPrintDailyMenu: function() {
            axios.get(`/api/booking_and_submission/print_daily_menu/${this.pg_data.f_pg_id}/${this.pg_data.f_date}/${this.pg_data.f_shift === 'shift-1' ? 1 : 2}/${this.pg.hangar.name}/${this.pg_data.f_type.toUpperCase()}`)
            .then(res => {
                if(res.data.success) {
