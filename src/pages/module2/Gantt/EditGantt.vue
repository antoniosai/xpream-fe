                ...
                <div class="card-header">
                    ...
                    <div class="option-box">
                         <!-- <div class="option-item">
                            <button class="button is-success" v-if="roles.length > 0 && loading.isLoading == false" v-on:click="handlerSaveGantt()">
                        ...
                        <div v-else>
                            <div>
                                ...
                                <div class="is-row">
                                    ...
                                    <div class="form-item is-col">
                                        <input disabled v-model="gantt.start_date"  v-on:change="handlerChangeStartDateGantt()" type="date" name="" id="gantt-start-date">
                                        <div class="is-desc">Start Date</div>
                                    </div>
                                    <div class="form-item is-col">
                                        <input disabled v-model="gantt.end_date" v-on:change="handlerChangeEndDateGantt()" type="date" name="" id="gantt-end-date">
                                        ...
                                        <tr>
                                            <td class="row-phase">
                                                <div style="margin-left: 10px;">
                                                    <i style="font-size: 16pt;" class='material-icons'> view_carousel </i> <span style="display: block; margin-top: -25px; margin-left: 25px;">
                                                        {{role.PHASE_NAME}}</span>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="row">
                                                    <div class="col-md-9">
                                                        <input v-model="role.START_DATE" v-on:change="handlerChangeStartDatePhase('phase-start-date-' +indexphase, 'phase-end-date-' +indexphase, 'task-start-date-' +indexphase)" class="phase-start-date" :id="'phase-start-date-' +indexphase" type="date" name="">
                                                    </div>
                                                    <div class="col-md-3">
                                                        <input v-model="role.START_TIME" type="time">
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="row">
                                                    <div class="col-md-9">
                                                        <input v-model="role.END_DATE" v-on:change="handlerChangeEndDatePhase('phase-start-date-' +indexphase, 'phase-end-date-' +indexphase, 'task-end-date-' +indexphase, 'task-start-date-' +indexphase)"  class="phase-end-date" :id="'phase-end-date-' +indexphase" type="date" name="">
                                                    </div>
                                                    <div class="col-md-3">
                                                        <input v-model="role.END_TIME" type="time">
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
                                    ...
                                    <tbody  v-for="(task, indextask) in role.TASK" :key="indextask">
                                        <tr>
                                            <td class="row-task">
                                                <div style="margin-left: 32px;"> 
                                                    <i style="font-size: 14pt;" class='material-icons'> widgets </i> <span style="display: block; margin-top: -23px; margin-left: 24px;">
                                                        {{task.TASK_NAME}}</span>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="row">
                                                    <div class="col-md-9">
                                                        <input :disabled="role.START_DATE === '' && role.END_DATE === ''" v-model="task.START_DATE" v-on:change="handlerChangeStartDateTask('task-start-date-' +indextask+indexphase, 'task-end-date-' +indextask+indexphase, 'area-start-date-' +indextask+indexphase )" :class="'task-start-date-' +indexphase" :id="'task-start-date-' +indextask+indexphase"  type="date" name="">
                                                    </div>
                                                    <div class="col-md-3">
                                                        <input :disabled="role.START_DATE === '' && role.END_DATE === ''" v-model="task.START_TIME" type="time">
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="row">
                                                    <div class="col-md-9">
                                                        <input :disabled="role.START_DATE === '' && role.END_DATE === ''" v-model="task.END_DATE" v-on:change="handlerChangeEndDateTask('task-start-date-' +indextask+indexphase, 'task-end-date-' +indextask+indexphase, 'area-end-date-' +indextask+indexphase, 'area-start-date-' +indextask+indexphase)" :class="'task-end-date-' +indexphase" :id="'task-end-date-' +indextask+indexphase" type="date" name="">
                                                    </div>
                                                    <div class="col-md-3">
                                                        <input :disabled="role.START_DATE === '' && role.END_DATE === ''" v-model="task.END_TIME" type="time">
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
                                        <tr v-for="(area, indexarea) in task.AREA" :key="indexarea">
                                                ...
                                                <td>
                                                    <div class="row">
                                                        <div class="col-md-9">
                                                            <input :disabled="task.START_DATE === '' && task.END_DATE === ''" v-model="area.START_DATE" v-on:change="handlerChangeStartDateArea('area-start-date-' +indexarea+indexphase, 'area-end-date-' +indexarea+indexphase, 'task-end-date-' +indextask+indexphase)" :class="'area-start-date-' +indextask+indexphase" :id="'area-start-date-' +indexarea+indexphase"  type="date" name="">
                                                        </div>
                                                        <div class="col-md-3">
                                                            <input :disabled="task.START_DATE === '' && task.END_DATE === ''" v-model="area.START_TIME" type="time">
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div class="row">
                                                        <div class="col-md-9">
                                                            <input :disabled="task.START_DATE === '' && task.END_DATE === ''" v-model="area.END_DATE" :class="'area-end-date-' +indextask+indexphase" :id="'area-end-date-' +indexarea+indexphase" type="date" name="">
                                                        </div>
                                                        <div class="col-md-3">
                                                            <input :disabled="task.START_DATE === '' && task.END_DATE === ''" v-model="area.END_TIME" type="time">
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
...
export default {
    ...
    methods: {
        ...
        handlerGanttRole: function(ac_type){
            axios.get('/api/booking_and_submission/show_gantt_data_by_version/' + this.$route.params.pg_id + '/' + this.$route.params.version + '/' + this.handlerMapAircraft(this.pg.ac_type))
            .then(res => {
                if(res.data.success) {
                    this.roles = res.data.data.detail_gantt;
                    this.gantt = res.data.data.data_gantt;
                }
                this.loading.isLoading = false;
            })
            .catch(e => {
        ...
        handlerSaveGantt: function() {
            ...
            swal({
                ...
                text: "This action will create a new version of existing gantt chart data",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
