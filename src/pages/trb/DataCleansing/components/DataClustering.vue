                ...
                >
                    <template slot="table-row" slot-scope="props">
                        ...
                        <span v-if="props.column.field == 'MAIN_TYPE'">
                            <select 
                                v-model="datatable.rows[props.index].MAIN_TYPE" 
                                :disabled="!$props.grant_access" :id="'validation-rev-clustering-' +props.index" 
                                v-on:change="handlerUpdateData(props.formattedRow['ID'], 'MAIN_TYPE', 'validation-rev-clustering-' +props.index)"
                            >
                                <option value="">Select Option</option>
                                <option value="1">Daily Check</option>
                                <option value="2">Service Check</option>
                                <option value="3">Weekly Check</option>
                                <option :value="'4' || '5'">A Check</option>
                                <option value="6">C Check</option>
                                <option value="7">D Check</option>
                                <option :value="'N' || 'SR'">SR Check</option>
                            </select>
                        </span>
...
import FieldRevision from "../datatable/FieldRevision";
...
export default {
    ...
    methods: {
        initData: function() {
            ...
            axios.get(`/api/data_cleansing/validation/revision_clustering/${this.$props.draft_id}`)
            .then(res => {
        ...
        handlerUpdateData: function(id, column, elem) {
            ...
            axios.post('/api/data_cleansing/validation/revision_clustering/update', {
                id: id,
                column: column,
                data: elm,
        ...
        handlerDeleteItem: function(id) {
            ...
            .then((value) => {
                if (value) {
                    this.loading.isLoading = true;
                    axios.get('/api/data_cleansing/validation/revision_clustering/delete/' + id)
                    .then(res => {
                        if(res.data.success) {
                            toastr.success(res.data.message);
                            this.initData();
