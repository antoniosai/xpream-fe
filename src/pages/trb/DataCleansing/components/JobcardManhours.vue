            ...
            <div>
                ...
                >
                    ...
                    <template slot="table-row" slot-scope="props">
                        <span v-if="props.column.field == 'MHRSSAVE'">
                            <input 
                                :value="props.formattedRow[props.column.field]"
                                type="number" 
                                name="main-type"
                                :id="'job-manhours-' +props.index" 
                                :disabled="!$props.grant_access"
                                v-on:change="handlerUpdateData(props.formattedRow['REFNO'], 'MHRSSAVE', 'job-manhours-' +props.index)"
                            >
                        </span>                        
...
import FieldJobManhours from "../datatable/FieldJobManhours";
...
export default {
    ...
    mounted() {
    if(this.$props.type == "manhours_jobcard") {
    ...
    methods: {
        initData: function() {
            ...
            if(this.$props.type == "manhours_jobcard") {
                FieldJobManhours[1].hidden = false;
                FieldJobManhours[2].hidden = false;
                FieldJobManhours[3].hidden = false;
                FieldJobManhours[6].hidden = false;
                FieldJobManhours[9].hidden = true;
            }else {
                FieldJobManhours[1].hidden = true;
                FieldJobManhours[2].hidden = true;
                FieldJobManhours[3].hidden = true;
                FieldJobManhours[6].hidden = true;       
                FieldJobManhours[9].hidden = false;
            }

            this.loading.isLoading = true;
            axios.get(`/api/data_cleansing/validation/job_manhours/${this.$props.draft_id}`)
            .then(res => {
        ...
        handlerDeleteItem: function(id) {
            ...
            .then((value) => {
                if (value) {
                    this.loading.isLoading = true;
                    axios.get('/api/data_cleansing/validation/job_manhours/delete/' + id)
                    .then(res => {
                        if(res.data.success) {
                            toastr.success(res.data.message);
                            this.initData();
        ...
        handlerGenerateInitial: function() {
            ...
            .then((value) => {
                if (value) {*/
                    ...
                    axios.get(`/api/data_cleansing/validation/job_manhours/generate_initial/GA01/${this.$props.draft_id}`)
                    .then(res => {
                        if(res.data.success) {
                            toastr.remove();
