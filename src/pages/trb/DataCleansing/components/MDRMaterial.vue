                    ...
                    <template slot="table-row" slot-scope="props">
                        <span v-if="props.column.field == 'PROB'">
                            <div class="flex-middle-default">
                                {{parseFloat(props.formattedRow[props.column.field]).toFixed(2)*100}} %
                            </div>
                        </span>
                        <span v-else-if="props.column.field == 'QMAT'">
                            <div class="flex-middle-default">
                        ...
                        <span 
                            v-else-if="
                                props.column.field == 'MEINS' || 
                                props.column.field == 'MTART' ||
                                props.column.field == 'EVENTMAT' ||
                                props.column.field == 'EVENTORDER' 
                            "
                        >
                            <div class="flex-middle-default">
...
import FieldJobMaterial from "../datatable/FieldJobMaterial";
...
export default {
    ...
    methods: {
        initData: function() {
            ...
            axios.get(`/api/data_cleansing/validation/mdr_material/get/GA02/${this.$props.draft_id}`)
            .then(res => {
        ...
        handlerUpdateData: function(id, column, elem) {
            ...
            axios.post('/api/data_cleansing/validation/mdr_material/update', {
                id: id,
                column: column,
                data: elm,
        ...
        handlerDeleteItem: function(id) {
            ...
            .then((value) => {
                if (value) {
                    this.loading.isLoading = true;
                    axios.get('/api/data_cleansing/validation/mdr_material/delete/' + id)
                    .then(res => {
                        if(res.data.success) {
                            toastr.success(res.data.message);
                            this.initData();
